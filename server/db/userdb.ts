...
export async function upsertProfile() {
  await db('users').insert(nickmae: .nickname).onConflict('auth0_id').merge()
}

export async function getFriends(userId: string) {
  return (await db('friendList').join('users', 'friendList.friend_id', 'users.auth0_id').select('users.auth0_id as id', 'nickname').where('user_id', userId))
}
